namespace EasyPrint.Command
{
    ...
    ///
    /// 客户端发送（单个）：
    /// <code>PAUSE {"printerName":"TSC TE200","jobIds":[5]}</code>
    ///
    /// 客户端发送（批量）：
    /// <code>PAUSE {"printerName":"TSC TE200","jobIds":[5,6,7]}</code>
    ///
    /// 响应示例：
    /// <code>
    /// {
    ...
    /// </code>
    /// </summary>
    internal class PAUSE : JsonCommandBase<JobControlRequest>
    {
            ...
            CancellationToken cancellationToken)
        {
            try
            {
                ...
                {
                    jobId = id,
                    ok    = PrinterHelper.PausePrintJob(printerName, id),
                }).ToList();
